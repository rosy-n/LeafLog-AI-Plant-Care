...
### 확정된 원인(suspected_cause) 24개 — 이 목록 외의 값을 만들어내지 않는다

```
응애벌레, 과습, 뿌리파리, 진딧물, 일소현상, 광 과다, 진드기,
깍지벌레, 흰가루병, 총채벌레,
잎마름병, 갈색무늬병, 탄저병, 세균성 점무늬병,
노균병, 잿빛곰팡이병, 다량원소 결핍, 냉해피해,
그을음병, 검은점무늬병, 점무늬병, 궤양병, 귤굴나방, 가루이
```

(2026-09-24: houseplant 2차 수집분(지은/미나 추가 30장) 라벨링 중 목록에 없는 값 3개 발견 —
`진사진딧물 성충`/`진사진딧물 약충`은 같은 종의 생육 단계 차이일 뿐이라 기존 `진딧물`로 흡수 병합
(`scripts/00_merge_labels.py`의 `MN_CAUSE_MAP`). `가루이`는 진딧물과 다른 계통의 별개 해충이고
실내식물에도 흔한 광범위 숙주 해충이라 새 원인으로 추가 — 23 → 24개.)

(2026-09-14: 2차 RAG — AI-Hub 525번·아열대 데이터 통합에 맞춰 14 → 23개로 확장. 웹서칭으로
확인한 결과 노균병/잿빛곰팡이병/그을음병/검은점무늬병/점무늬병은 기주 범위가 넓어 실내식물에도
흔하고, 다량원소 결핍/냉해피해는 병원균이 아닌 생리장해라 식물 종을 안 가림. 궤양병/귤굴나방은
감귤속(Citrus)에 국한된 병해충이지만 레몬·한라봉 등 감귤류도 화분에서 흔히 기르므로 채택함
— 상세 근거는 `docs/crop-data-plan.md` "소스 3" 절 참고. `ai/diagnosis/scripts/08_ingest_aihub_subtropical.py`의
`PENDING_CONFIRMATION`은 이 확정으로 제거됨.
참고: 아래 2026-07-05 메모의 "점무늬병 1건 병합"은 그 시점 실내식물 데이터 한정 처리이고, 이번에
새로 추가한 "점무늬병"은 별개의 독립 값이다 — 앞으로도 이 둘을 서로 병합하지 않는다.)
