...
# 원본 라벨은 원칙적으로 병합하지 않는다. 예외로 확정된 것만 여기 추가한다.
# - '점무늬병'(1건) -> '세균성 점무늬병'(4건, 원인까지 구체적으로 기록됨)으로 흡수 병합.
# - '진사진딧물 성충'/'진사진딧물 약충' -> '진딧물'로 흡수 병합. 같은 종의 생육 단계(성충/약충)
#   차이일 뿐이라 별도 카테고리로 둘 필요가 없음(2026-09-24 확정).
#   '가루이'는 진딧물과 다른 계통의 별개 해충이라 병합하지 않고 config/cause_codes.json에
#   새 원인으로 추가함(같은 날 확정).
MN_CAUSE_MAP = {
    "점무늬병": "세균성 점무늬병",
    "진사진딧물 성충": "진딧물",
