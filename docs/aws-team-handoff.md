...
작성: 소윤 · 기준일: 2026-09-12 (전환 완료) · 대상: 팀 전체
...
**2026-09-11 22:37 KST AWS 전환 완료.** 앱은 AWS API만 사용한다. 실제 주소·접속값은 팀 공지와 비공개 운영 기록 참고.

| 항목 | 운영 상태 |
|---|---|
| 코드 | API·학교 worker 모두 `develop`의 같은 커밋을 배포. 배포는 커밋을 EC2 실행 폴더와 학교 worker 폴더에 동기화한 뒤 API → worker 순서로 재시작 |
| EC2 API | 서비스 `leaflog-api`, 설정 `/etc/leaflog/api.env`, 실행 폴더 `/srv/leaflog/api/apps/api`, `127.0.0.1:8000` → Nginx HTTPS |
| Qdrant | 컨테이너 `leaflog-qdrant`, EC2 내부 `127.0.0.1:6333`, 진단 참고 사례 191건 |
| RDS | 운영 DB `leaflog` (9/11 학교 최종 덤프 복원, 원본 건수 일치). 리허설 DB는 9/12 최종 백업 후 정리 |
| S3 | 이전 사진·캐릭터 `leaflog/migrated/`, 새 캐릭터 `leaflog/characters/`, 일지 `leaflog/diary/`, 진단 사진 `diagnosis/`, 진단 참고 이미지 `rag-reference/<id>.jpg` |
| 학교 PC | 서비스 `leaflog-ai-worker`, 설정 `/etc/leaflog/worker.env`. Windows Tailscale 18010 → WSL 8010. SQS는 IAM Roles Anywhere(인증서 2026-12-31 만료, 12월 초 갱신 협의) |
| 옛 학교 API | 학교 PC의 `leaflog-api`는 중지·자동시작 해제. 다시 켜면 옛 DB에 기록되므로 켜지 않음 |
| HTTPS | 6일짜리 공인 IP 인증서, `leaflog-certbot-renew.timer`가 4시간마다 자동 갱신. 9/12 실제 갱신·Nginx 재적용 확인 |
| 사진 형식 | 진단·캐릭터 생성 모두 JPG·PNG·WebP·HEIC. 서버가 모델 입력을 JPEG로 변환 |
| 재부팅 | EC2 재부팅 후 전 서비스 자동 복구 확인. 학교 PC 재부팅 복구는 현장 시험 필요(18010 포트 전달 재확인) |

**팀원 필수:** `apps/mobile/.env.local`에 `EXPO_PUBLIC_API_BASE_URL=https://<AWS API 주소>`. 없으면 앱이 개발 PC의 로컬 API로 붙어 옛 학교 DB에 기록된다(9/8~9/9 식물 17~19가 이렇게 생김).

아래 1~13절은 이전 당시의 절차 기록이다. 다시 이전·복원할 때만 참고하고, 운영 중인 자원에 설치·복원 명령을 그대로 실행하지 않는다. 절차에 나오는 `-rehearsal` 서비스·설정 이름은 위 운영 이름으로 바뀌었다.
...
**생성 재개 순서:** API의 S3 읽기/쓰기·SQS 발신 권한, 학교의 SQS 수신/삭제/가시성 갱신 권한, callback HTTPS·토큰을 확인 → **API 설정을 먼저 `true`로 변경하고 API 재시작** → 학교 설정을 `true`로 변경하고 worker 재시작 → 큐 접근 오류가 없는지 확인 → 테스트 계정으로 후보 3개 생성·선택·등록 검증. 양쪽 최종 설정은 동일해야 함. API가 꺼진 채 worker만 켜면 `claim`이 503을 받아 worker가 `School job processing deferred (RuntimeError)`로 재시도만 반복한다(2026-09-11 확인).
