...
-- 국명 검색 보강용 별칭 — 사용자가 실제로 입력하는 유통명/원예명을 종에 연결한다.
-- plant_species.common_name_ko 는 소스(NATURE_KNA/KFS_STD) 기준의 정식 국명이라 사용자가
-- 쓰는 이름과 어긋나는 경우가 많고, RDA_INDOOR 유래 행 중 47건은 국명 자리에 영문명이 들어와 있다.
--   예) 입력 '테이블야자' → 마스터 국명 'parlour palm' (Chamaedorea elegans)
--       입력 '금전수'     → 마스터 국명 '금전초'       (Zamioculcas zamiifolia)
-- 값은 위키에서 가져온다 (ko.wikipedia 표제어·넘겨주기 + Wikidata 한국어 라벨·별칭).
-- plant_species_image 와 같은 결로 배치 병합(2-3) 대상이 아니다 — data_source 에도 넣지 않는다.
--   1) scripts/ingest/wiki_ko_alias.py 로 미리 채워두고 (돌봄 정보 보유 종 우선)
--   2) 그래도 못 맞춘 검색어는 GET /api/species 가 그 자리에서 위키에 물어 채운다
--      (app/wiki_names.py → app/main.py 의 _resolve_query_via_wiki)
CREATE TABLE plant_species_alias (
    ...
    -- 검색 키 — 소문자 + 공백/가운뎃점 제거 ('테이블 야자' 와 '테이블야자' 를 같게 본다)
...
CREATE UNIQUE INDEX uq_plant_species_alias_miss
    ON plant_species_alias (alias_norm)
    WHERE species_id IS NULL;
CREATE INDEX idx_plant_species_alias_norm ON plant_species_alias (alias_norm);
-- 오타/표기 변형 유사도 검색용 ('떡갈고무나무' → '떡갈잎고무나무')
CREATE INDEX idx_plant_species_alias_trgm
