...
액세서리는 화분 없는 투명 효과 이미지 하나를 목록 카드와 캐릭터 레이어에 함께 쓴다.
배경은 배경 이미지 하나를 쓴다. 둘 다 item.asset_id 에 연결한다.

실행:
  cd apps/api
  ./.venv/Scripts/python.exe scripts/set-item-image.py <item_key> <s3_url> [checksum]
...
def main() -> None:
    if len(sys.argv) < 3:
        print("사용법: set-item-image.py <item_key> <s3_url> [checksum]")
        raise SystemExit(1)

    item_key, url = sys.argv[1], sys.argv[2]
