...
def _rag_reference_image_urls(image_ids: list[int], db: Session) -> dict[int, str]:
    ...
    rows = db.execute(
        select(
            MediaAsset.object_key, MediaAsset.asset_id, MediaAsset.file_url, MediaAsset.bucket_name
        ).where(
    ...
    url_by_key = {
        row.object_key: presigned_get_url(row.object_key, row.bucket_name) or row.file_url for row in rows
    }
    return {
...
def list_plants(
    ...
    if plant_ids:
        ...
        for pid, position_key, item_key, object_key, file_url, bucket_name in rows_deco:
