...
배경음악(`bgm-main.mp3`) 1개와 효과음 5개가 들어간다. `bgm-main.mp3` 는 아직
무음 placeholder고, 효과음 5개는 전부 실제 음원이 들어가 있다.
...
다섯 개 모두 실제 음원이 들어가 있다. 바꾸고 싶으면 **같은 이름으로 덮어쓰면**
코드 수정 없이 그대로 반영된다.
...
| `sfx-typing.wav` | 페르소나 대화창에서 대사가 타이핑되는 동안 글자 3개마다 1회 | `sfx-pet.wav` 와 같은 Kenney `glass_002` | 0.13초 |
...
페르소나 대화창에서 캐릭터 대사가 타이핑되는 동안 나는 효과음이다. 동물의숲
말풍선처럼 **빠르게 4번 재생하고 살짝 쉬었다가 다시 4번** 을 반복한다 — 매 글자마다
재생하면 소리가 밀려서 부자연스럽고, 일정한 간격으로만 재생하면 기계적으로 들린다.

재생 로직은 `src/screens/PlantDetailScreen.jsx` 의 `TYPING_SFX_BURST_CHARS`(4글자,
매 글자 재생) / `TYPING_SFX_PAUSE_CHARS`(1글자, 재생 안 함) 가 한 주기(5글자)로
반복되며 `playSfx("typing")` 을 부른다. 지금은 `sfx-pet.wav` 와 같은 음원
(`glass_002`, 0.13초)을 그대로 재사용한다.

버스트 구간은 글자당 35ms 간격으로 재생하는데 음원 길이(0.13초)가 더 길어서,
버스트 안에서는 앞소리가 채 끝나기 전에 다음 소리가 겹쳐 들린다 — 이게 오히려
"타다다닥" 하는 빠른 느낌을 낸다. 너무 지저분하면 0.1초 이하의 더 짧은 음원으로
바꾸거나 `TYPING_SFX_BURST_CHARS`/`TYPING_SFX_PAUSE_CHARS` 비율을 조정한다.

다른 "틱" 효과음(예: 동물의숲 스타일 타이핑 사운드)으로 바꾸고 싶으면 같은 이름
(`sfx-typing.wav`)으로 덮어쓰면 된다. 확장자를 바꾼다면 `src/feedback.ts` 의
`SFX_SOURCES` 도 함께 고친다.
