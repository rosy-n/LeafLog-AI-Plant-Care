...
네 개 모두 실제 음원이 들어가 있다. 바꾸고 싶으면 **같은 이름으로 덮어쓰면**
코드 수정 없이 그대로 반영된다.
...
처음엔 `tick_001` 을 넣었는데 너무 얇고 날카로웠다. 팩 33개를 재어 보니
`tick_001` 은 밝기 7760Hz · 음정감 16 으로 후보 중 가장 높고 소음에 가까웠다.
지금 쓰는 `glass_002` 는 밝기 1904Hz · 음정감 388 로, 4배 부드러우면서
음정이 또렷한 유리 종소리다.

더 따뜻한 쪽을 원하면 **`bong_001`** (밝기 239Hz · 음정감 514 · 0.13초) 이
팩에서 가장 낮고 부드럽다. 파일만 덮어쓰면 된다.

문지르는 동안 **최소 0.13초 간격**으로 불리므로 그보다 긴 음원은 앞소리가
잘린다. 위 두 개는 정확히 0.13초라 딱 맞는다.
