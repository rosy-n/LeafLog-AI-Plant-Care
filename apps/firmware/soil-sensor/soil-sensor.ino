...
// 흙은 하루 7~15% 마른다 = 시간당 3~6mV 로 ADC 노이즈와 비슷한 폭이라,
// 그보다 자주 재면 노이즈만 쌓인다. soil_sensor.report_interval_sec 와 같은 값.
static uint32_t reportIntervalSec = 3600;
...
// 상한은 API 의 배치 상한과 같은 288건(1시간 주기로 12일치). 그보다 오래
// 끊기면 가장 오래된 것부터 버린다 — 최근 값이 물주기 판단에 더 중요하다.
//
// 전원이 나가면 버퍼도 사라진다. USB 로 상시 급전하는 전제라 NVS 에 쓰지 않는다.
// ---------------------------------------------------------------------------
